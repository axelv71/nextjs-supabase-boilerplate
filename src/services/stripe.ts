...
export async function manageSubscription(
  ...
  if (createAction && subscription.discount) {
    // Check if the promotional code is assigned to a user
    const { data: promotionalCode, error: promotionalCodeError } =
      await supabase
        .from('promotional_codes')
        .select('*')
        .eq('id', subscription.discount.promotion_code as string)
        .limit(1)
        .order('created_at', { ascending: false });

    if (promotionalCodeError) {
      console.error('Error retrieving promotional code:', promotionalCodeError);
    }

    if (promotionalCode && promotionalCode.length > 0) {
      await linkAffiliate(
        promotionalCode[0].referer_id ?? '',
        customer.id,
        'purchase',
      );
    }
  ...
  const { error } = await supabase.from('subscriptions').upsert([
    {
      ...
      trial_end: subscription.trial_end
        ...
        : null,
      promotional_code: subscription.discount?.promotion_code
